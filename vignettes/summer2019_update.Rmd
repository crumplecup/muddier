...
In a similar manner to how Miller & Burnett used landslide runout track surveys to calibrate a topological index to an empiric record of debris flows, we can use the transect record of valley sediment deposition to calibrate predicted delivery probabilities in the Miller/Burnett model.  Comparison of cross-sectional area at transects to Miller/Burnett delivery probabilities shows better correlation between delivery probabilties and sediment deposition for Bear Creek than for Knowles Creek. The correlation is generally stronger for Bear Creek examining the relation between delivery probabilities and slope, contributing area, valley width, and area-slope.
...
points(bear$DebrisFlow, bear$GRADIENT,
       col = rgb(red=.5, green=0, blue=.7, alpha=.5), pch=20)
abline(lm(transects$GRADIENT ~ transects$DebrisFlow),col='blue')
abline(lm(bear$GRADIENT ~ bear$DebrisFlow),col='purple')
legend('topright', legend = c('knowles', 'bear'), fill = c('blue', 'purple'))
...
points(bear$DebrisFlow, bear$contr_area/1000000,
       col = rgb(red=.5, green=0, blue=.7, alpha=.5), pch=20)
abline(lm(transects$contr ~ transects$DebrisFlow),col='forestgreen')
abline(lm(bear$contr_area/1000000 ~ bear$DebrisFlow),col='purple')
legend('topright', legend = c('knowles', 'bear'), fill = c('forestgreen', 'purple'))

area_slope <- transects$GRADIENT * transects$contr
area_slope_br <- bear$GRADIENT * bear$contr_area/1000000
plot(transects$DebrisFlow, area_slope,
     col = rgb(red=1, green=0, blue=.2, alpha=.5), pch=20,
     ylab='Contr Area * Slope',xlab='Delivery Probability' )
points(bear$DebrisFlow, area_slope_br,
       col = rgb(red=.5, green=0, blue=.7, alpha=.5), pch=20)
abline(lm(area_slope ~ transects$DebrisFlow),col='coral3')
abline(lm(area_slope_br ~ bear$DebrisFlow),col='purple')
legend('topright', legend = c('knowles', 'bear'), fill = c('coral3', 'purple'))
